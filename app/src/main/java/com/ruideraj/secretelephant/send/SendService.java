...
public class SendService extends Service {
    ...
    private List<SendListener> mListeners = new ArrayList<>();
    ...
    public void resend(int position) {
        if(mInvites != null && position < mInvites.size()) {
            ...
            for(SendListener listener : mListeners) {
    ...
    public class SendBinder extends Binder {
        ...
        public void sendInvites(MatchExchange exchange) {
            SendService.this.send(exchange);
        }

        public void resendInvite(int position) {
            SendService.this.resend(position);
        }

        public void registerListener(SendListener listener) {
            mListeners.add(listener);
        }

        public void unregisterListener(SendListener listener) {
    ...
    private class SendListTask implements Runnable {
        ...
        public void run() {
            ...
            GoogleSignInAccount googleAccount = GoogleSignIn.getLastSignedInAccount(SendService.this);
        ...
        public void run() {
            ...
            if(mInvite.getContact().getType() == Contact.TYPE_PHONE) {
                ...
                if(BuildConfig.DEBUG) {
                    destination = PropertiesReader.getProperty(SendService.this,
                            getString(R.string.test_send_key_phone));
                }
                else {
                ...
                if(mGmail != null) {
                    ...
                    try {
                        ...
                        if(BuildConfig.DEBUG) {
                            recipient = PropertiesReader.getProperty(SendService.this,
                                    getString(R.string.test_send_key_email));
                        }
                        else {
                            // TODO Add logic to use real email
                        }
                        email.addRecipient(javax.mail.Message.RecipientType.TO, new InternetAddress(recipient));
                        email.setSubject(getString(R.string.app_name));
                    ...
                    exception = new Exception(getString(R.string.send_error_no_gmail));
                }
            }
